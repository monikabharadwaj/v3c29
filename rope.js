class Rope
  ...
  const group = Body.nextGroup(true);
   //Returns the next unique group index for which 
      //bodies will collide. If isNonColliding is true, 
      //returns the next unique group index for which 
      //bodies will not collide. 
  ...
  Create a new composite containing bodies created 
  in the callback in a grid arrangement. 
  This function uses the body's bounds to prevent overlaps.  

  Filter
  If the two bodies have the same non-zero value of collisionFilter.group, 
  they will always collide if the value is positive, 
  and they will never collide if the value is negative.
  */  
  this.pointA = pointA;
  this.body = Composites.chain(
    rects, 0.1, 0, -0.6, 0,
     {stiffness: 0.1, length: 0.1,
       render: {type: 'line'}});
